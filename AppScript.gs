  ...
  return data.slice(1)
    ...
    .map(row => {
      ...
      headers.forEach((h, i) => {
        ...
        // Clean Rides data: strip units from dist, convert decimal duration
        if (opts.cleanRide) {
          if (h === 'dist') {
            strVal = strVal.replace(/\s*km\s*/i, '').trim();
          }
          if (h === 'duration') {
            // If it's a decimal fraction of a day (Google Sheets time), convert
            const num = parseFloat(strVal);
            if (!isNaN(num) && num > 0 && num < 1) {
              const totalSec = Math.round(num * 86400);
              const hh = Math.floor(totalSec / 3600);
              const mm = Math.floor((totalSec % 3600) / 60);
              const ss2 = totalSec % 60;
              strVal = hh + ':' + String(mm).padStart(2,'0') + ':' + String(ss2).padStart(2,'0');
            }
          }
        }
        obj[h] = strVal;
...
function forceSeed() {
  ...
  clearAndFill('Rides', 7, _seedRides);
